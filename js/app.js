...
async function exportEvaluations() {
    ...
    const agentSelect = document.getElementById('agent-select-admin');
    const groupSelect = document.getElementById('group-select-admin'); // DÜZELTME 1: Grup seçimi alındı
    
    const targetAgent = agentSelect ? agentSelect.value : 'all';
    const targetGroup = groupSelect ? groupSelect.value : 'all'; // DÜZELTME 2: Grup değeri okundu

    // Temsilci Adı Gösterimi (Rapor Onayı için)
    ...
    try {
        const response = await fetch(SCRIPT_URL, {
            ...
            body: JSON.stringify({
                ...
                targetAgent: targetAgent,
                targetGroup: targetGroup, // DÜZELTME 3: targetGroup backend'e gönderiliyor
                username: currentUser,
                token: getToken()
            })
        });
