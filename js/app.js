...
let wizardStepsData = {};
let qualityChartInstance = null; // YENİ: Grafik Instance'ı

// --- KALİTE PUANLAMA LOGİĞİ (Slider ve Renklendirme) ---
...
// --- KALİTE FONKSİYONLARI (GÜNCELLENEN DASHBOARD KISMI) ---
function openQualityArea() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const value = `${monthStr}.${yearStr}`; // "05.2023" formatı
    ...
    if (isAdminMode) {
        fetchUserListForAdmin().then(users => {
            ...
            // Varsayılan olarak "Tüm Temsilciler" seçili ve dashboard yüklenir
            fetchEvaluationsForAgent('all');
        });
    } else {
        // Normal kullanıcı sadece kendi verisini görür
        fetchEvaluationsForAgent(currentUser);
    }
}

// Donma sorununu çözen ana fonksiyon: Grafik ve Sadece ilk 10 liste elemanını basar
...
function updateDashboardUI() {
    const monthFilter = document.getElementById('month-select-filter').value; // Örn: "05.2023"
    
    // 1. Veriyi Ay Bazlı Filtrele
    const filtered = allEvaluationsData.filter(item => {
        if(!item.date) return false;
        // item.date formatı "dd.mm.yyyy" varsayılıyor.
        // Eğer "yyyy-mm-dd" ise ona göre ayırın. Mevcut sistemde dd.mm.yyyy kullanılıyor.
        const parts = item.date.split('.'); 
        if (parts.length >= 3) {
             // parts[1] ay, parts[2] yıl
    ...
    if(count === 0) {
        ...
        // En yeniden eskiye sırala ve ilk 10'u al
        // Not: filtered listesi sunucudan genelde karışık gelebilir, callId veya tarihe göre sıralamak iyidir
        // Basitçe diziyi ters çevirip alıyoruz (varsayım: sunucu eskiden yeniye gönderiyor)
        const sortedList = filtered.slice().reverse().slice(0, 10);
    ...
    // 5. Grafik Çizimi (Chart.js)
    const ctx = document.getElementById('qualityChart').getContext('2d');
    if (qualityChartInstance) qualityChartInstance.destroy();
    
    // Grafiği soldan sağa (eskiden yeniye) çizmek için tarih sıralaması yap
    const sortedForChart = filtered.slice().sort((a,b) => {
        let da = a.date.split('.').reverse().join(''); // yyyymmdd
    ...
    qualityChartInstance = new Chart(ctx, {
        type: 'line',
        data: {
            labels: sortedForChart.map(d => d.date.split('.').slice(0,2).join('/')), // dd/mm
            datasets: [{
                label: 'Kalite Puanı',
...
// --- LOG EVALUATION (Değerlendirme Ekleme - Admin) ---
async function logEvaluationPopup() {
    ...
    // HTML Oluşturma (Criteria veya Manuel)
